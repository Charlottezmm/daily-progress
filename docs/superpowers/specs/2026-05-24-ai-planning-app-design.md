...
- 家务、做饭、洗澡、运动、通勤等 routine 会挤占容量，但不应该被 AI 当成 task 重排。
- 学习、工作、副线、恢复多条战线会互相侵蚀，需要 track balance 监测。
- 放松/recovery 是红线，不应被 AI 为了“完成更多任务”自动挤掉。
- 上午/下午/晚上能量不同，AI 排程必须考虑 energy × time segment 的匹配。
...
- AI patch 不能移动 routine。
- AI patch 不能把 task 移入 routine 对应的 time block。
...
- AI patch 不能移动、缩短或覆盖 recovery block。
...
- 手动：用户点击 “Re-plan today” 或 “Re-plan week”。
...
- Settings：workspace password、DeepSeek key、routines、recovery target、segment energy、track thresholds、导入导出、hosted limits。
...
- AI patch 试图修改 routine/recovery：拒绝 patch，并显示违反的约束。
- 导入解析失败：尽量指出失败的行或段落。
...
- AI patch 禁止修改 routine/recovery 的校验。
...
- 标记任务未完成。
- 使用 Re-plan today 生成 reschedule preview。
- 使用 Re-plan week 生成 reschedule preview。
- 取消一个 patch item。
- 应用剩余 patch item。
