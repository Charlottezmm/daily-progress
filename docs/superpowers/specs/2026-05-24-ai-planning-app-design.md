...
它要解决的问题是：用户可以导入某个月或好几个月的计划，导入固定课程表/不可用时间，由系统拆成每日和每周计划；每天记录真实完成情况后，Cowork/Codex/Claude 可以通过 MCP 读取真实数据、提出结构化调整方案，并把关键对话沉淀写回工具。
...
重排必须是 patch-based。工具自身不调用 LLM；Cowork/Codex/Claude 通过 MCP 读取数据、推理、生成 patch，并调用 `propose_patch` 写回工具。

触发方式：

- 默认：每日 check-in/复盘后，用户在 Cowork/Codex 请求重排。
- 手动：用户点击 Web/PWA 里的 “Re-plan today” 或 “Re-plan week”，页面提示通过 Cowork/Codex 执行重排。
- Agent：用户在 Cowork/Codex 里说“重排今天/本周”，agent 通过 MCP 读取上下文并调用 `propose_patch`。
...
- `Re-plan today`：只处理今天剩余时间，必要时顺延到明天。
- `Re-plan week`：处理当前周剩余时间。
- Daily check-in 后默认进入 `Re-plan week`，但 UI 可以让用户改成 `Re-plan today`。
...
2. 用户在 Cowork/Codex 请求重排，或在 Web/PWA 点击重排入口后按提示去 Cowork/Codex。
...
- Month：月度计划和任务分布。
- Inbox：quick capture 缓冲池。
- Projects：按项目筛选的任务视图。
- Courses：课程表和课程相关任务视图。
- Import：`plan.md` 和 `timetable.csv` 导入流程。
- Reschedule Preview：agent patch 审批。
- Settings：workspace password、MCP token、routines、recovery target、segment energy、track thresholds、导入导出、hosted limits。

导航上 Today 和 Week 应该是主入口。Projects 和 Courses 是筛选/上下文页面，不是独立计划根。

### 11.1 Quick Capture / Inbox

任何主页面顶部都有 `+ Quick Capture`。
...
Daily Check-in 的目标是 5 秒完成，不做成复盘子系统。

Today 视图底部固定一个 Daily Check-in 卡片，永远可见。它不要求用户打开子页面，不要求选日期，不要求在 check-in 里标记 task done；task 完成状态仍然由 task checkbox 管。
...
- Track balance 数据计算。
- Routine 管理和 Today routine 区域。
- Recovery block 和 weekly target。
- Inbox quick capture。
- Daily check-in。
- 5 秒 Daily Check-in 卡片：完成 / 卡点 / 明日接。
- MCP-shaped reschedule patch preview。
- Re-plan today。
- Re-plan week。
- Reschedule Preview。
...
- 在 Week / Month 视图看到 check-in 历史摘要。
- 标记任务未完成。
- 通过模拟 MCP propose_patch 创建 Re-plan today preview。
- 通过模拟 MCP propose_patch 创建 Re-plan week preview。
- 取消一个 patch item。
- 应用剩余 patch item。
